...
import {Component, Scope} from '@es-injection/decorators';
import {ClassConstructor, ScopeType} from '@es-injection/metadata';
...
describe('Default application context', () => {
    ...
    describe('throws an exception when', () => {
        ...
        it('registering a component that is not decorated', () => {
            ...
            class TestClass { }
            // expect
            expect(() => applicationContext.registerComponentClass(TestClass)).toThrowError(`class ${TestClass.name} does not appear to be a component`);
        });
        ...
        it('attempting to get a component using a class that is not registered', async () => {
            ...
            // expect
            expect(applicationContext.getComponent(TestClass)).rejects.toThrowError(`component class ${TestClass.name} has not been registered in this context`);
        });
        ...
        it('attempting to get a component that has multiple implementations', async () => {
            ...
            expect(componentRegistry.registerComponent).toHaveBeenCalledWith(undefined, TestDerivedClass1);
            expect(componentRegistry.registerComponent).toHaveBeenCalledWith(undefined, TestDerivedClass2);
            expect(applicationContext.getComponent(TestClass)).rejects.toThrowError(`component class ${TestClass.name} has more than one registered implementation: ${TestDerivedClass1.name}, ${TestDerivedClass2.name}`);
        });
        ...
        it('attempting to get components using a class that is not registered', async () => {
            ...
            componentRegistry.containsComponentClass.mockReturnValueOnce(false);
            // expect
            expect(applicationContext.getComponents(TestClass)).rejects.toThrowError(`component class ${TestClass.name} has not been registered in this context`);
        });
        ...
        it('attempting to get components when there are no implementations', async () => {
            ...
            componentRegistry.resolveComponentClass.mockReturnValueOnce(new Set<ClassConstructor<any>>());
            // expect
            expect(applicationContext.getComponents(TestClass)).rejects.toThrowError(`no implementation classes have been registered for class ${TestClass.name}`);
        });
